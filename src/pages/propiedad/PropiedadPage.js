...
function Propiedad() {
    ...
    useEffect(() => {
        fetch('http://localhost:80/localidades')
            .then(response => response.json())
            .then(localidad => setLocalidad(localidad.data))
            .catch(error => console.error('Error fetching data:', error));
    }, []);
    ...
    useEffect(() => {
        fetch('http://localhost:80/tipos_propiedad')
            .then(response => response.json())
            .then(propiedad => setPropiedad(propiedad.data))
            .catch(error => console.error('Error fetching data:', error));
    }, []);
    ...
    const handleSubmit = (event) => {
        ...
        if (formData.fechaDesde) query.append('fecha_inicio_disponibilidad', formData.fechaDesde);
    ...
    return (
        <div>
            ...
            <button type="button" id="nuevo" className="boton">
                <a href="http://localhost:3000/crear">Crear propiedad</a>
            </button>
            <FooterComponent />
        </div>
    );
